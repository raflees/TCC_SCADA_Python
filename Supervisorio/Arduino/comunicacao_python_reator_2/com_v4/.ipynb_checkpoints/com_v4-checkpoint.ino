...
void loop() {
  ...
  if (Serial.available()) {
    if (!flag){
      ...
      x2 = ((Cb1-x2)*u1/x1+(Cb2-x2)*u2/x1-aux)*(Ts_simul/60.)+x2;
      if (x2<0){
        x2 = 0;
      } 
      x3 = ((-x3)*u1/x1+(-x3)*u2/x1+aux)*(Ts_simul/60.)+x3;
      if (x3<0){
        x3 = 0;
      }
      ...
      ts2 = millis();
      //delay(ceil(Ts*1000. - (float) (ts2-ts1)));
            
      Serial.print(x1 + (float) random(-2,2)/15.,2);
      Serial.print('&');
      Serial.print(x2 + (float) random(-1,1)/10.,2);  
      Serial.print('&');
      Serial.print(x3 + (float) random(-1,1)/20.,2);
      Serial.print('&');
      Serial.print(u1,2);
      Serial.print('&');
      Serial.print(u2,2);
